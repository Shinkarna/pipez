...
public class ServerConfig extends ConfigBase {
    ...
    public final ForgeConfigSpec.BooleanValue itemPipeBackoffEnabled;
    public final ForgeConfigSpec.IntValue itemPipeBaseBackoffTicks;
    public final ForgeConfigSpec.IntValue itemPipeMaxBackoffTicks;

    public ServerConfig(ForgeConfigSpec.Builder builder) {
        super(builder);

        builder.comment(" Item Pipe Backoff",
                " When an item pipe has nothing to extract, it will wait an",
                " increasing number of ticks before trying again, reducing CPU",
                " usage on idle pipe networks.",
                " Enabled by default. Set to false to disable.");
        builder.push("item_pipe_backoff");
        itemPipeBackoffEnabled = builder
                .comment("Enable exponential backoff for idle item pipes")
                .define("enabled", true);
        itemPipeBaseBackoffTicks = builder
                .comment("Initial backoff delay in ticks when pipe is idle")
                .defineInRange("base_delay", 2, 1, 1200);
        itemPipeMaxBackoffTicks = builder
                .comment("Maximum backoff delay in ticks")
                .defineInRange("max_delay", 32, 1, 1200);
        builder.pop();
