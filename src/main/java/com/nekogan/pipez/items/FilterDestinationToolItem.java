package com.nekogan.pipez.items;
...
public class FilterDestinationToolItem extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
        tooltip.add(Component.translatable("tooltip.pipez_ce.filter_destination_tool").withStyle(ChatFormatting.GRAY));
        DirectionalPosition dst = getDestination(stack);
        if (dst != null) {
            tooltip.add(Component.translatable("tooltip.pipez_ce.filter_destination_tool.destination", number(dst.getPos().getX()), number(dst.getPos().getY()), number(dst.getPos().getZ()), Component.translatable("message.pipez_ce.direction." + dst.getDirection().getName()).withStyle(ChatFormatting.GREEN)).withStyle(ChatFormatting.GRAY));
        }
